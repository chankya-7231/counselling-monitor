# MCC Counselling Website Monitor — Setup Guide

This guide will walk you through setting up a free, automatic system that
checks 5 MCC (Medical Counselling Committee) website pages every 22
minutes and sends a WhatsApp message and an email to you and two other
people the moment anything changes on those pages. (KNRUHS is tracked
separately, by its own repository, so a problem with one tracker can
...
free computers (not yours), checks the 5 pages every 22 minutes, remembers
what it saw last time, and texts/emails 3 people if anything is different.

**One thing to know up front:** MCC's website blocks anything that looks
like an automated browser, so this monitor deliberately uses a plain,
simple request instead (the same way your browser's "View Page Source"
works) rather than a full browser. That makes it very reliable and hard
to get blocked, but it means it only sees the page's normal content, not
anything the page adds afterward with JavaScript.
...
- About 20-30 minutes, done once

---

## Step 1: Your repository

You've already created the empty `counselling-monitor` repository on
GitHub — nothing more to do for this step.
...
6. Under "App name," type something like `MCC Monitor` and click
   **Create**.
...
> **Note:** if you already set this up for your KNRUHS monitor, you can
> either reuse the exact same Gmail account and App Password here, or
> create a second one — either works fine, since this is a separate
> repository with its own separate secrets either way.
...
> **Already did this for KNRUHS?** The same phone number + API key pair
> works here too — CallMeBot keys aren't tied to a specific repository.
...
repository. **These are separate from your KNRUHS repository's secrets —
you need to add them here too, even if you already set them up there.**

1. Open your `counselling-monitor` repository on GitHub.
...
|---|---|
| `GMAIL_ADDRESS` | The Gmail address you made/used in Step 2, e.g. `yourname@gmail.com` |
...
```
git add .
git commit -m "Add MCC counselling monitor"
git push
```
...
2. On the left, click **MCC Counselling Website Monitor**.
...
4. Wait about 20-40 seconds, then refresh the page. You'll see a new run
...
- Every day around 8:00 AM IST, if nothing changed on any of the 5 pages,
...
- The moment any of the 5 pages changes — new text, a new link, a new
  PDF, a new announcement — everyone gets an alert immediately, any time
  of day or night.
...
checked all 5 counselling websites and found no changes since the last
...
**A page seems to be blocked/down and no alert was sent**
- This is expected and safe behaviour — if a page is temporarily
  unreachable (or MCC's bot-protection blocks the request), the monitor
  simply logs it and tries again in 22 minutes. It will never send a
  false "changed" alert just because a page was briefly unreachable.
  If it stays unreachable for about 3 hours straight (8 checks in a
  row), you'll get one "Site Unreachable" email for that page, then one
  "Site Reachable Again" email once it recovers.

**I want to test again after the first run**
- Go to Actions → MCC Counselling Website Monitor → Run workflow, and
...
You're all set! The system will now quietly watch these 5 pages around the
